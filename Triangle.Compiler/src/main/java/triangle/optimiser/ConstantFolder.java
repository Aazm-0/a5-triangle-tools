...
public class ConstantFolder implements ActualParameterVisitor<Void, AbstractSyntaxTree>,
    ...
    @Override
    public AbstractSyntaxTree visitPostFixCommand(PostFixCommand command, Void unused) {
        return null;
    }

    @Override
    public AbstractSyntaxTree visitDoWhileCommand(DoWhileCommand command, Void unused) {
        return null;
    }
...
//	}
	...
	public AbstractSyntaxTree foldBinaryExpression(AbstractSyntaxTree node1, AbstractSyntaxTree node2, Operator o) {
		...
		if ((node1 instanceof IntegerExpression) && (node2 instanceof IntegerExpression)) {
            ...
            // Arithmetic operators: result integer
            ...
            // Boolean Result
				...
				return createBooleanExpression((Boolean) foldedValue,node1);
			}
		}
    ...
    public VnameExpression createBooleanExpression(boolean value,AbstractSyntaxTree position){
        Identifier i = new Identifier(
                value ? "true" : "false",
                position.getPosition()
        );

        i.decl = value ? StdEnvironment.trueDecl : StdEnvironment.falseDecl;

        SimpleVname vn = new SimpleVname(i,position.getPosition());
        VnameExpression vnE = new VnameExpression(vn,position.getPosition());
